...
from utils.preprocess import compute_windows, preprocess_image
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, imgfiles, labelfiles, path=Path('./labels.cache'), prefix=''):
        ...
        x = {}  # 字典
        nm, nf, ne, nc = 0, 0, 0, 0  # 缺失、找到、空、重复的数量
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        l = [x.split() for x in f.read().strip().splitlines()]
                        if any([len(x) > 8 for x in l]):  # 是否为实例分割
                            classes = np.array([x[0] for x in l], dtype=np.float32)
                            segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in l]  # (类别, xy1...)
                            l = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (类别, xywh)
                        l = np.array(l, dtype=np.float32)
                    if len(l):
                        assert l.shape[1] == 5, 'labels require 5 columns each'
                        assert (l >= 0).all(), 'negative labels'
...
class TileDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.windows = compute_windows(self.image, patch_size, patch_overlap)
