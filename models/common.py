...
class MF(nn.Module):  # stereo attention block
    def __init__(self, c1, c2):
        ...
        self.mask_map_r = nn.Conv2d(c1, 1, 1, 1, 0, bias=True)
        self.mask_map_i = nn.Conv2d(1, 1, 1, 1, 0, bias=True)
        self.softmax = nn.Softmax(-1)
        self.bottleneck1 = nn.Conv2d(1, 8, 3, 1, 1, bias=False)
        self.bottleneck2 = nn.Conv2d(c1, 24, 3, 1, 1, bias=False)
        self.se = SE_Block(c2, 16)
        # self.se_r = SE_Block(3,3)
        # self.se_i = SE_Block(1,1)
        self.downsample = nn.Conv2d(c2, c2, kernel_size=3, stride=2, padding=1)

    def forward(self, x):  # B * C * H * W #x_left, x_right
        x_left_ori, x_right_ori = x[0], x[1][:, 0:1, :, :]
...
class Add(nn.Module):
    ...
    def forward(self, x):
        # x_add=torch.add(x[0], x[1])
        # x_add=x_add.detach().cpu().numpy()[0]
        # np.save('mul_out_x2',x_add)
        # x_add=np.mean(x_add,axis=0)
        # from matplotlib import pyplot as plt
        # plt.imsave('x_add.png', x_add, dpi=300)
...
class Add2(nn.Module):
    ...
    def forward(self, x):
        if self.index == 0:
            x1=torch.add(x[0], x[1][0])
            x1=x1.detach().cpu().numpy()[0]
            np.save('dom_in_x1.npy', x1)
            x1=np.mean(x1,axis=0)
            x2=x[0].detach().cpu().numpy()[0]
            x2=np.mean(x2,axis=0)
            np.save('dom_in_x2.npy', x[0].detach().cpu().numpy())

            x3=x[1][0].detach().cpu().numpy()[0]
            x3=np.mean(x3,axis=0)
            from matplotlib import pyplot as plt
            plt.imsave('x1.png',x1,dpi=300)
            plt.imsave('x2.png',x2,dpi=300)
            plt.imsave('x3.png',x3,dpi=300)
            return torch.add(x[0], x[1][0])
        elif self.index == 1:
            x4=torch.add(x[0], x[1][1])
            x4=x4.detach().cpu().numpy()[0]
            np.save('dsm_in_x1.npy', x4)
            x4=np.mean(x4,axis=0)
            x5=x[0].detach().cpu().numpy()[0]
            x5=np.mean(x5,axis=0)
            np.save('dsm_in_x2.npy', x[0].detach().cpu().numpy())

            x6=x[1][1].detach().cpu().numpy()[0]
            x6=np.mean(x6,axis=0)
            from matplotlib import pyplot as plt
            plt.imsave('x4.png',x4,dpi=300)
            plt.imsave('x5.png',x5,dpi=300)
            plt.imsave('x6.png',x6,dpi=300)
...
class CMAFF(nn.Module):
    ...
    def forward(self, x):
        ...
        fused_feat = enhanced_diff_feat + refined_common_feat
        return fused_feat
...
class Concat_fusion(nn.Module):
    ...
    def forward(self, x):
        # x1: [N, C1, H, W]
        # x2: [N, C2, H, W]
        x = torch.cat([x[0], x[1]], dim=1)  # [N, C1+C2, H, W]
        return self.conv(x)  # [N, Cout, H, W]
