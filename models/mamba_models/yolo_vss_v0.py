...
class SS2Dm0:
    ...
    def forward_corem0(
        ...
        xs1 = cross_scan_fn(x1.view(B, H, W, RD), in_channel_first=False, out_channel_first=False,
                            scans=_scan_mode,
                            force_torch=scan_force_torch)  # (B, H, W, 4, D)
        # 1. 将 A 和 B 在 dim=0 上堆叠，得到形状为 (2, 3, 6400, 4, 128) 的张量
        stacked = torch.stack([xs0, xs1], dim=0).contiguous()  # 形状: (2, 3, 6400, 4, 128)

        # 2. 调整维度顺序，将 dim=1 和 dim=2 交换
        # 目的是将 6400 和 2 这两个维度放在一起
        permuted = stacked.permute(1, 2, 0, 3, 4).contiguous()  # 形状: (3, 6400, 2, 4, 128)

        # 3. 合并 dim=1 和 dim=2，得到交替合并的结果
        xs = permuted.reshape(B, -1, 4, RD).contiguous()  # 形状: (3, 12800, 4, 128)
...
class VSSNet(nn.Module):
    def __init__(
            ...
            # =============================
            _SS2D: type = SS2D,
            num_blocks: int = 2,  # 添加 num_blocks 参数
            vert_anchors=8,
            horz_anchors=8,
            **kwargs,
    ):
...
if __name__ == "__main__":
    ...
    model = VSSNet(256, drop_path=0.0, norm_layer=nn.LayerNorm, channel_first=False, ssm_d_state=64, ssm_ratio=1,
                     ssm_dt_rank="auto",
                     ssm_conv_bias=False, ssm_drop_rate=0.0, ssm_init='v2', forward_type='m0_noz', mlp_ratio=4.0, ).to(
        device)
    model1=VSSNet(256).to(device)
    y = model1(x)
    print(y[0].shape)
    print(y[1].shape)
