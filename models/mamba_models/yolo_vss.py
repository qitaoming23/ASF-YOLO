...
class SS2Dm0:
    def __initm0__(
            ...
            # basic dims ===========
            d_model=512,
            d_state=1,  # now with mamba2, dstate should be bigger...
            ssm_ratio=1,
            dt_rank="auto",
            act_layer=nn.GELU,
            # dwconv ===============
        ...
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
        d_inner = int(ssm_ratio * d_model) #dmodel输入维度 d_inner第一个线性层维度
        dt_rank = math.ceil(d_model / 16 / 2) if dt_rank == "auto" else dt_rank
        ...
        if self.with_dconv:
            ...
            # x proj ============================
        self.x_proj = [
            nn.Linear(d_inner // 2, (dt_rank + d_state * 2), bias=False)
            for _ in range(k_group)
        ]
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0))  # (K, N, inner)
        del self.x_proj
        ...
        if initialize in ["v1"]:
            ...
            # simple init dt_projs, A_logs, Ds
            self.Ds = nn.Parameter(torch.ones((k_group, dt_rank, int(d_inner // dt_rank // 2))))
    ...
    def forward_corem0(
        ...
        x0 = x[:, :, :, 0:RD]
        x1 = x[:, :, :, RD:]
        xs0 = cross_scan_fn(x0.view(B, H, W, RD), in_channel_first=False, out_channel_first=False,
                            scans=_scan_mode,
                            force_torch=scan_force_torch)  # (B, H, W, 4, D) 四个方向扫描
        xs1 = cross_scan_fn(x1.view(B, H, W, RD), in_channel_first=False, out_channel_first=False,
                            scans=_scan_mode,
                            force_torch=scan_force_torch)  # (B, H, W, 4, D)
        # 1. 将 A 和 B 在 dim=0 上堆叠，得到形状为 的张量
        stacked = torch.stack([xs0, xs1], dim=0).contiguous()  # 形状: （2,B，H*W，4,RD）

        # 2. 调整维度顺序，将 dim=1 和 dim=2 交换
        # 目的是将hw和2 这两个维度放在一起
        permuted = stacked.permute(1, 2, 0, 3, 4).contiguous()  # 形状: （B,H*W，2,4，RD）

        # 3. 合并 dim=1 和 dim=2，得到交替合并的结果
        xs = permuted.reshape(B, -1, 4, RD).contiguous()  # 形状: (B, 2*hw, 4, RD)
        ...
        ys0 = ys[:, ::2, :, :].contiguous()  # 提取偶数索引的数据
        ys1 = ys[:, 1::2, :, :].contiguous()  # 提取奇数索引的数据
        y0: torch.Tensor = cross_merge_fn(ys0.view(B, H, W, K, RD), in_channel_first=False, out_channel_first=False,
                                          scans=_scan_mode, force_torch=scan_force_torch)
        y1: torch.Tensor = cross_merge_fn(ys1.view(B, H, W, K, RD), in_channel_first=False, out_channel_first=False,
                                          scans=_scan_mode, force_torch=scan_force_torch)
        ...
        y = torch.cat([y0, y1], dim=-1).contiguous()
        y = self.out_norm(y.view(B, H, W, -1).to(torch.float32))

        return y.to(x.dtype)

    def forwardm0(self, x: list, **kwargs):
        x_c_fuse = self.in_proj(x)
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1))  # (b, h, w, d)
            if not self.disable_z_act:
                z = self.act(z)
        if self.with_dconv:
            x_c_fuse = self.conv2d(x_c_fuse)  # (b, d, h, w)
        x = self.act(x + x_c_fuse)
        y = self.forward_core(x)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: list):
        ...
        if isinstance(x, list):
            x = torch.cat(x, dim=1)
        x = x.permute(0, 2, 3, 1).contiguous() #b*h*w*2c
        if self.ssm_branch:
            if self.post_norm:
...
class VSSNet(nn.Module):
    def __init__(
            ...
            # =============================
            _SS2D: type = SS2D,
            num_blocks: int = 1,  # 添加 num_blocks 参数
            vert_anchors=8,
            horz_anchors=8,
            **kwargs,
    ):
        super().__init__()
        # 使用 nn.Sequential 串联 8 个 VSSBlock
        self.blocks = nn.Sequential(*[
            ...
            for _ in range(num_blocks)  # 创建 num_blocks 个 VSSBlock
        ])
    ...
    def forward(self, input: list):
        rgb_feature=input[0]
        dsm_feature=input[1]
        _, __, h, w = rgb_feature.shape
        x=[self.avgpool(rgb_feature),self.avgpool(dsm_feature)]
        x=self.blocks(x)
        y_split = list(torch.chunk(x, chunks=2, dim=1))
        rgb_fea_out=y_split[0]
        dsm_fea_out=y_split[1]
        rgb_fea_out = F.interpolate(rgb_fea_out, size=([h, w]), mode='bilinear')
        dsm_fea_out = F.interpolate(dsm_fea_out, size=([h, w]), mode='bilinear')
        y=[rgb_fea_out, dsm_fea_out]
        return y
if __name__ == "__main__":
    device = torch.device("cuda:0")
    x = torch.randn(32, 56, 56, 96).to(device)

    x0 = torch.randn(3, 256, 40, 40).to(device)
    x1 = torch.randn(3, 256, 40, 40).to(device)
    x = [x0, x1]
    model = VSSNet(256, drop_path=0.0, norm_layer=nn.LayerNorm, channel_first=False, ssm_d_state=64, ssm_ratio=1,
                     ssm_dt_rank="auto",
                     ssm_conv_bias=False, ssm_drop_rate=0.0, ssm_init='v2', forward_type='m0_noz', mlp_ratio=0.0, ).to(
        device)
    y = model(x)
    print(y[0].shape)
    print(y[1].shape)
