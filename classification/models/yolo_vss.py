...
class VSSNet(nn.Module):
    def __init__(
            ...
            # =============================
            _SS2D: type = SS2D,
            num_blocks: int = 8,  # 添加 num_blocks 参数
            vert_anchors=8,
            horz_anchors=8,
            **kwargs,
    ):
...
if __name__ == "__main__":
    ...
    x0 = torch.randn(8, 256, 40, 40).to(device)
    x1 = torch.randn(8, 256, 40, 40).to(device)
    x = [x0, x1]
    model = VSSNet(256, drop_path=0.0, norm_layer=nn.LayerNorm, channel_first=False, ssm_d_state=64, ssm_ratio=1,
                     ssm_dt_rank="auto",
                     ssm_conv_bias=False, ssm_drop_rate=0.0, ssm_init='v2', forward_type='m0_noz', mlp_ratio=1, ).to(
        device)
